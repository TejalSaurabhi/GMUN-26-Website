...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    // Request a WebGL2 context with alpha enabled so the canvas is transparent
    // and the page background (body or container) shows through instead of black.
...
export default function InfiniteMenu({ items = [] }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", marginTop: '100px' }}>
      ...
      <div
        className={`drag-instruction ${isMoving ? "hidden" : ""}`}
        aria-hidden={isMoving ? "true" : "false"}
      >
        <span className="drag-text">
          <small style={{ opacity: 0.8, display: "block", fontWeight: 500 }}>
            Click & Drag
          </small>
        </span>
      </div>
